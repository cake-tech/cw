...
extension StringProtocol {
    func trimLeadingAndTrailingWhitespace() -> Self.SubSequence {
        guard count > 0 else {
            return self[startIndex..<endIndex]
        }
        //find the start index
        var indexStart = startIndex
        let length = index(indexStart, offsetBy:count)
        var computedStart = false
        while indexStart < length && computedStart == false {
            if self[indexStart].isWhitespace {
                indexStart = index(after: indexStart)
            } else {
                computedStart = true
            }
        }
        //find the end index
        var indexEnd = index(before:endIndex)
        var computedEnd = false
        while indexEnd >= indexStart && computedEnd == false {
            if self[indexEnd].isWhitespace {
                indexEnd = index(before:indexEnd)
            } else {
                computedEnd = true;
            }
        }
        //return the new start and end range
...
extension TextRecord: ResourceRecord {
    init(deserialize data: Data, position: inout Data.Index, common: RecordCommonFields) throws {
        ...
        while position < endIndex {
            ...
            let attr = label.split(separator:";", omittingEmptySubsequences: true).map { String($0.trimLeadingAndTrailingWhitespace()) }

            for (_, curItem) in attr.enumerated() {
                let splitItem = curItem.split(separator: "=", omittingEmptySubsequences: false)
                if (splitItem.count == 2) {
                    attrs[String(splitItem[0])] = String(splitItem[1])
                }
            }
        }
